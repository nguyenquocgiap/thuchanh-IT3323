...
void freeType(Type* type) {
  ...
  // AN TOÀN: Không giải phóng kiểu Int và Char toàn cục để tránh lỗi crash
  // khi giải phóng các biến trỏ đến intType/charType dùng chung.
  if (type->typeClass == TP_INT || type->typeClass == TP_CHAR) return;
...
Object* createFunctionObject(char *name) {
  ...
  // Tạo scope mới cho hàm, cha là scope hiện tại
...
Object* createProcedureObject(char *name) {
  ...
  // Tạo scope mới cho thủ tục, cha là scope hiện tại
...
void freeReferenceList(ObjectNode *objList) {
  ...
  while (list != NULL) {
    ObjectNode* nextNode = list->next;
    free(list); // Chỉ giải phóng Node, không giải phóng Object
    list = nextNode;
  }
}
